...
uint256 hashGenesisBlock("0xd15ff8c64fd622e8fba5dfa150f2c8d528fc561c634524b3f7dc1c72d4aa15f9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Polybit: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Polybit Signed Message:\n";
...
{
    // Polybit: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1){
        nSubsidy = nSubsidy * 150000000 * 10  / 100 / 100;
     }

    nSubsidy >>= (nHeight / 675000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Polybit: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Polybit: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Polybit: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd15ff8c64fd622e8fba5dfa150f2c8d528fc561c634524b3f7dc1c72d4aa15f9");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1429635469;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087024448;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Polybit: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PolybitMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PolybitMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PolybitMiner(CWallet *pwallet)
{
    printf("PolybitMiner:\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("polybit-miner");
    ...
    try { loop {
        ...
        printf("Running Polybit with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PolybitMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PolybitMiner, pwallet));
}
